    ...
    def index(self):
        ...
        for json_file in json_files:
            ...
            for item in range(len(predict_labels)):
                ...
                self.index_feature.append(feature)
                self.index_bbox.append(bbox)
                self.index_id.append(image_id)
        print("#items in database:", len(self.index_feature))

    def __calculate_similarity(self, query_feature):
        if len(self.index_id) == 0:
            return TEMPLATE["gallery_image_id"], TEMPLATE["gallery_bbox"]
        query_feature = np.array(query_feature)
        database_feature = np.array(self.index_feature)
        ...
        gallery_image_id = [self.index_id[i] for i in sorted_ids]
        gallery_bbox = [self.index_bbox[i] for i in sorted_ids]
        ...
        while not self.query_que.empty():
            ...
            if len(query_dict["bbox"]) == 0:
                ...
                for qitem in range(len(query_labels)):
                    ...
                    gallery_image_id, gallery_bbox = self.__calculate_similarity(query_feature)
                    q_result = {
