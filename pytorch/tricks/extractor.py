...
from base.data.dataloader import get_loader
...
def extract_image_feature(args):
    ...
    try:
        ...
        return 
    logger.info("Extracting {} feature.".format(df.MODEL_NAME))

    query_dataloader = get_loader(args.query_data, args.batch_size, False)
    gallery_dataloader = get_loader(args.gallery_data, args.batch_size, False)
...
'''bash
CUDA_VISIBLE_DEVICES=1 python extractor.py --checkpoint_path=output/model.pth
'''