...
from base.net.loss import triplet_loss, soft_triple_loss
        ...
        for i, gpu in enumerate(GPUs):
            with tf.device("/gpu:%d" % gpu):
                ...
                with tf.variable_scope("", reuse=tf.AUTO_REUSE) as scope:
                    anchor_feature, anchor_logits, large_centers = model(anchor_splits[i], is_training=True)
                    pos_feature, pos_logits, _ = model(pos_splits[i], is_training=True)
                    neg_feature, neg_logits, _ = model(neg_splits[i], is_training=True)
                    ## loss
                    ...
                    st_anchor_loss = soft_triple_loss(
                        label_splits[i],
                        anchor_logits,
                        large_centers
                    )
                    st_neg_loss = soft_triple_loss(
                        label_splits[i],
                        neg_logits,
                        large_centers
                    )
                    logits_loss = 0.5 * (st_anchor_loss + st_neg_loss)
