...
with tf.Graph().as_default(), tf.device('/cpu:0'):
    try:
        ...
        image_splits = tf.split(image, num_or_size_splits=num_gpu, axis=0)
        ...
        for i, gpu in enumerate(GPUs):
            with tf.device("/gpu:%d" % gpu):
                ...
                with tf.variable_scope("", reuse=tf.AUTO_REUSE) as scope:
                    feature, logits, large_centers = model(image_splits[i], is_training=True)
                    ## loss
                    st_loss = soft_triple_loss(
                        label_splits[i],
                        logits,
                        large_centers
                    )
                    regularization_loss = tf.add_n(tf.losses.get_regularization_losses())
                    total_loss = st_loss + args.weight_decay * regularization_loss
                    ## accuracy
                    image_accuracy = compute_accuracy(logits, label_splits[i])
                    ## model summary
                    ...
                    tower_accu.append(image_accuracy)
        ...
        ## tf.summary, loss & train_accuracy are tracked from the last tower
        summaries.append(tf.summary.scalar("learning_rate", lr_schedule))
        summaries.append(tf.summary.scalar("loss/all_loss", loss_avg))
        summaries.append(tf.summary.scalar("loss/st_loss", st_loss))
        summaries.append(tf.summary.scalar("train_accuracy", image_accuracy))
    ...
    train_data = load_train_pair(global_batch_size)
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        for itr in range(start_itr, max_itr+1):
            ...
            _, batch_tl, batch_accu, add, lr = sess.run(
                fetches=[
                    train_op,
                    st_loss,
                    accu_avg,
            ...
            assert not np.isnan(batch_tl), 'Model diverged with loss = NaN'
            train_dict={
                "softtriple_loss": batch_tl,
