...
def load_train_triplet(batch_size=1, load_num=None):
    ...
    train_data = train_data.shuffle(1000).map(preprocess_triplet).batch(batch_size).prefetch(tf.data.experimental.AUTOTUNE)
    return train_data
...
def load_train_pair(batch_size=1, load_num=None):
    ...
    train_data = train_data.shuffle(1000).map(preprocess_pair).batch(batch_size).prefetch(tf.data.experimental.AUTOTUNE)
    return train_data
...
def load_train_pair_with_two_targets(batch_size=1, load_num=None):
    ...
    train_data = train_data.batch(batch_size).shuffle(1000).prefetch(tf.data.experimental.AUTOTUNE)
    return train_data
